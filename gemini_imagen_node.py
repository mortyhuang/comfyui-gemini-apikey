...
class GeminiNanoBananaPro_APIKey:
    """
    Nano Banana Pro with hidden API Key (loaded from config).
    Designed for clean UI and ComfyUI-Manager compatibility.
    """
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "model": (["gemini-3-pro-image-preview", "Nano Banana 2 (Gemini 3.1 Flash Image)"], {"default": "gemini-3-pro-image-preview"}),
                "seed": ("INT", {"default": 42, "min": 0, "max": 2147483647, "control_after_generate": True}),
                "aspect_ratio": ([
                    "auto", "1:1", "2:3", "3:2", "3:4", "4:3", "4:5", "5:4", "9:16", "16:9", "21:9"
                ], {"default": "auto"}),
    ...
    def execute(self, prompt, model, seed, aspect_ratio, resolution, response_modalities, images=None, files=None, system_prompt=""):
        actual_key = get_api_key()
        if not actual_key:
            raise ValueError(
                "Gemini API Key missing!\n"
        ...
        if model == "Nano Banana 2 (Gemini 3.1 Flash Image)":
            ...
            gemini_model = model # gemini-3-pro-image-preview
...
NODE_CLASS_MAPPINGS = {
    "GeminiNanoBanana2_APIKey": GeminiNanoBanana2_APIKey,
    "GeminiNanoBananaPro_APIKey": GeminiNanoBananaPro_APIKey
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "GeminiNanoBanana2_APIKey": "Nano Banana 2 (API Key)",
    "GeminiNanoBananaPro_APIKey": "Nano Banana Pro (API Key)"
}
